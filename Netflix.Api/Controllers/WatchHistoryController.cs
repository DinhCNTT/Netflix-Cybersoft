...
namespace Netflix.Api.Controllers
{
    ...
    public class WatchHistoryController : ControllerBase
    {
        ...
        public WatchHistoryController(ApplicationDbContext dbContext, ITmdbService tmdbService)
        {
        ...
        public async Task<IActionResult> UpsertWatchHistory([FromBody] UpdateWatchHistoryRequest request)
        {
            try
            {
                ...
                if (!movieExists)
                {
                    ...
                    if (tmdbMovie == null)
                    {
                        return BadRequest(new { status = "error", message = "Movie không tồn tại trên TMDB." });
                    }
